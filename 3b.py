...
class P:
    ...
    def move(self, s):
        ...
        l = int(s[1:])
        if d == 'R':
            other = P(self.x+l, self.y, self.d+l)
            return 0, other, l
        elif d == 'L':
            other = P(self.x-l, self.y, self.d+l)
            return 0, other, l
        elif d == 'U':
            other = P(self.x, self.y+l, self.d+l)
            return 1, other, l
        elif d == 'D':
            other = P(self.x, self.y-l, self.d+l)
            return 1, other, l
...
def intersect(a,b,c,d):
    xb = sorted([a.x,b.x])
    yb = sorted([c.y,d.y])
    if xb[0] <= c.x <= xb[1] and yb[0] <= a.y <= yb[1]:
        d = a.d + abs(c.x - a.x) + c.d + abs(c.y - a.y)
        return P(c.x, a.y, d)
    return None
...
a,b = adat[0][0]
c,d = bdat[1][0]
print(a,b,c,d, intersect(a,b,c,d))

iarr = []

for xline, yline in ((adat[0], bdat[1]),(bdat[0], adat[1])):
    for a,b in xline:
        for c,d in yline:
            p = intersect(a,b,c,d)
            if p is not None and not (p.x==p.y==0):
                print(p)
                iarr.append(p.len())
print(min(iarr))
