...
class P:
    ...
    def move(self, s):
        ...
        l = int(s[1:])
        if d == 'R':
            other = P(self.x+l, self.y)
            return 0, other, self, other
        elif d == 'L':
            other = P(self.x-l, self.y)
            return 0, other, other, self
        elif d == 'U':
            other = P(self.x, self.y+l)
            return 1, other, self, other
        elif d == 'D':
            other = P(self.x, self.y-l)
            return 1, other, other, self
    def len(self):
...
def intersect(a,b,c,d):
    if a.x <= c.x <= b.x and c.y <= a.y <= d.y:
        return P(c.x, a.y)
    return None
